...
HOMEBREW_CRON_ENTRY='0 */6 * * * /usr/local/bin/brew update &>/dev/null'
if ! crontab -l >/dev/null 2>&1; then
    echo "$HOMEBREW_CRON_ENTRY" | crontab -
else
    if ! (crontab -l | fgrep -q "$HOMEBREW_CRON_ENTRY"); then
        (crontab -l 2>/dev/null; echo "$HOMEBREW_CRON_ENTRY") | crontab -
    fi
fi
